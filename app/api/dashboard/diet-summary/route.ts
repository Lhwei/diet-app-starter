  ...
  try {
    ...
    while (hasMore && pageCount < 30) {
      const queryBody: Record<string, any> = {
        sorts: [{ property: dietRecordDateProp, direction: 'descending' }],   // 改用真正的 date 欄位排序
        page_size: 100,
      }
      ...
      hasMore = data.has_more && oldestInPage &&
        new Date(oldestInPage.recordDate ?? oldestInPage.createdTime) > sinceDate
      cursor = data.next_cursor
      pageCount++
      }

      const filtered = allRecords.filter(
        (r) => new Date(r.recordDate ?? r.createdTime) > sinceDate
      )
